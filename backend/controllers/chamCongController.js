...
const checkIn = async (req, res) => {
  try {
    ...
    console.log('[checkIn] Employee lookup:', { employee_did, found: !!employee, phong_ban_id: employee?.phong_ban_id });
    if (!employee) {
      console.log('[checkIn] Employee not found:', employee_did);
      return res.status(404).json({ message: 'Employee not found' });
    }
    // Check if phong_ban_id exists and is not null/empty
    if (!employee.phong_ban_id || employee.phong_ban_id.trim() === '') {
      console.log('[checkIn] BLOCKED - Employee has no department:', { employee_did, phong_ban_id: employee.phong_ban_id });
      return res.status(400).json({ 
        message: 'Bạn chưa thuộc phòng ban nào. Vui lòng tham gia phòng ban trước khi chấm công.' 
      });
    }
    console.log('[checkIn] ALLOWED - Employee has department:', { employee_did, phong_ban_id: employee.phong_ban_id });
...
const checkOut = async (req, res) => {
  try {
    ...
    const HoSoNhanVien = require('../models/HoSoNhanVien');
    const employee = await HoSoNhanVien.findOne({ employee_did });
    console.log('[checkOut] Employee lookup:', { employee_did, found: !!employee, phong_ban_id: employee?.phong_ban_id });
    if (!employee) {
      console.log('[checkOut] Employee not found:', employee_did);
      return res.status(404).json({ message: 'Employee not found' });
    }
    // Check if phong_ban_id exists and is not null/empty
    if (!employee.phong_ban_id || employee.phong_ban_id.trim() === '') {
      console.log('[checkOut] BLOCKED - Employee has no department:', { employee_did, phong_ban_id: employee.phong_ban_id });
      return res.status(400).json({ 
        message: 'Bạn chưa thuộc phòng ban nào. Vui lòng tham gia phòng ban trước khi chấm công.' 
      });
    }
    console.log('[checkOut] ALLOWED - Employee has department:', { employee_did, phong_ban_id: employee.phong_ban_id });
