...
// Initialize Department Contract Service (if configured)
setTimeout(async () => {
  try {
    ...
    if (departmentContractService.hasDepartmentContractConfig()) {
      console.log('[Server] Initializing Department Contract Service...');
      await departmentContractService.initializeDepartmentContractService();
      console.log('[Server] Department Contract Service initialized successfully');
    } else {
      console.log('[Server] Department Contract Service not configured. Using off-chain mode.');
    }
  } catch (error) {
    console.warn('[Server] Failed to initialize Department Contract Service:', error.message);
    console.warn('[Server] System will continue in off-chain mode.');
  }
}, 4000);
...
const departmentWeb3Controller = require('./controllers/departmentWeb3Controller');
const testController = require('./controllers/testController');
// Voting controller removed - only test is used now
...
// Department Web3 routes
app.get('/api/web3/departments', departmentWeb3Controller.getAllDepartments);
app.post('/api/web3/departments', departmentWeb3Controller.createDepartment);
app.post('/api/web3/departments/join', departmentWeb3Controller.joinDepartment);
app.get('/api/web3/employees/:employeeDid/departments', departmentWeb3Controller.getEmployeeDepartments);
// IMPORTANT: Specific routes must come before parameterized routes
app.get('/api/web3/departments/:departmentId/qualification/:employeeDid', departmentWeb3Controller.checkQualification);
app.get('/api/web3/departments/:id/members', departmentWeb3Controller.getDepartmentMembers);
// DELETE must come before GET with same path pattern
app.delete('/api/web3/departments/:id', (req, res, next) => {
  console.log('[Server] DELETE /api/web3/departments/:id route matched!', req.params);
  next();
}, departmentWeb3Controller.deleteDepartment);
app.get('/api/web3/departments/:id', departmentWeb3Controller.getDepartmentById);
console.log('[Server] Department Web3 routes registered: DELETE /api/web3/departments/:id');

// Test routes
// CRITICAL: import-json route MUST come FIRST, before any parameterized routes
// Test route to verify it's accessible
app.get('/api/web3/test/import-json/test', (req, res) => {
  res.json({ message: 'Test route is accessible' });
});

// This route MUST be registered before any /api/web3/test/:param routes
app.post('/api/web3/test/import-json', 
  (req, res, next) => {
    console.log('[Server] POST /api/web3/test/import-json route matched!', {
      method: req.method,
      path: req.path,
      url: req.url,
  ...
  (req, res, next) => {
    console.log('[Server] After multer:', {
      hasFile: !!req.file,
  ...
  testController.importQuestionsFromJSON
);
app.post('/api/web3/test/question', testController.addQuestion);
// Place more specific routes before parameterized routes to avoid conflicts
// Test route to verify routing
app.get('/api/web3/test/questions-admin/test', (req, res) => {
  res.json({ message: 'Admin questions route is working', route: '/api/web3/test/questions-admin/test' });
});
app.get('/api/web3/test/questions-admin/:id', testController.getDepartmentQuestionsForAdmin);
app.get('/api/web3/test/questions/:id', testController.getDepartmentQuestions);
app.get('/api/web3/test/question/:departmentId/:questionId', testController.getQuestionWithAnswer);
app.delete('/api/web3/test/question/:departmentId/:questionId', testController.deleteQuestion);
app.post('/api/web3/test/submit', testController.submitTest);
app.get('/api/web3/test/department/:id', testController.getDepartmentTestResults);
// Place most generic route last
app.get('/api/web3/test/:departmentId/:employeeDid', testController.getTestResult);
app.post('/api/web3/test/record', testController.recordTestScore); // Simplified - backward compatibility
console.log('[Server] Test admin route registered: GET /api/web3/test/questions-admin/:id');
console.log('[Server] Test routes registered: POST /api/web3/test/import-json');

// Voting routes removed - only test is used now
