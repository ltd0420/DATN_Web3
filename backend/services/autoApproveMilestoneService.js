...
const autoApproveMilestone100 = async (taskId) => {
  try {
    console.log(`[Auto Approve] 🔄 Bắt đầu auto-approve (hoàn thành sau 2 giờ) cho task ${taskId}...`);
    ...
    if (rewardInfo.tien_thuong > 0) {
      ...
      console.log(`[Auto Approve] AUTOMATIC PAYMENT INITIATED (2 hours timeout)`);
    ...
    await EventLogsUser.create({
      ...
      message: `Công việc "${task.ten_cong_viec}" đã được tự động phê duyệt sau 2 giờ (admin không phê duyệt kịp).`,
      resource_type: 'cong_viec_giao',
      resource_id: task.task_id,
      timestamp: new Date()
    });
    ...
    await AuditLogs.create({
      ...
      resource_id: task.task_id,
      status: 'Success',
      details: 'Tự động phê duyệt công việc sau 2 giờ ở trạng thái Chờ review',
      timestamp: new Date()
    });
...
// Tạo timer để tự động approve sau 2 giờ
const scheduleAutoApprove = (taskId, submittedAt = null) => {
  ...
  if (remainingTime <= 0) {
    ...
    console.log(`[Auto Approve] ⚡ Task ${taskId} đã quá thời gian auto-approve, approve ngay lập tức`);
  ...
  console.log(`[Auto Approve] ⏰ Đã tạo timer tự động phê duyệt cho task ${taskId}, sẽ approve sau ${Math.ceil(remainingTime / 1000 / 60)} phút`);
};
...
const restorePendingTimers = async () => {
  try {
    console.log(`[Auto Approve] 🔄 Đang khôi phục các timer auto-approve (2 giờ)...`);
