...
function AttendanceHistory({ user, employeeData }) {
  ...
  // Debug: Log employeeData when it changes
  useEffect(() => {
    console.log('[AttendanceHistory] employeeData changed:', {
      hasEmployeeData: !!employeeData,
      phong_ban_id: employeeData?.phong_ban_id,
      employee_did: employeeData?.employee_did || user?.employee_did,
      allKeys: employeeData ? Object.keys(employeeData) : []
    });
  }, [employeeData, user]);
  ...
  const handleCheckIn = async (qrCodeId = null) => {
    try {
      ...
      console.log('[AttendanceHistory] handleCheckIn - employeeData:', { 
        hasEmployeeData: !!employeeData, 
        phong_ban_id: employeeData?.phong_ban_id,
        employee_did: employeeData?.employee_did || user?.employee_did
      });
      if (!employeeData?.phong_ban_id) {
        console.warn('[AttendanceHistory] Check-in blocked: No department');
        setError('Bạn chưa thuộc phòng ban nào. Vui lòng tham gia phòng ban trước khi chấm công.');
  ...
  const handleCheckOut = async (qrCodeId = null) => {
    try {
      ...
      // Check if employee has a department
      console.log('[AttendanceHistory] handleCheckOut - employeeData:', { 
        hasEmployeeData: !!employeeData, 
        phong_ban_id: employeeData?.phong_ban_id,
        employee_did: employeeData?.employee_did || user?.employee_did
      });
      if (!employeeData?.phong_ban_id) {
        console.warn('[AttendanceHistory] Check-out blocked: No department');
        setError('Bạn chưa thuộc phòng ban nào. Vui lòng tham gia phòng ban trước khi chấm công.');
      ...
      <Box sx={{ p: { xs: 1.5, sm: 2 } }}>
        ...
        {!employeeData?.phong_ban_id && (
          <Alert severity="warning" sx={{ mb: 2 }} icon={<WarningIcon />} size="small">
            Bạn chưa thuộc phòng ban nào. Vui lòng tham gia phòng ban trước khi chấm công.
          </Alert>
        )}
        {shouldShowLateWarning && checkinLockEnabled && employeeData?.phong_ban_id && (
