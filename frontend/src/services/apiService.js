...
class ApiService {
  constructor() {
    ...
    // Create a separate axios instance for optional requests (404 is expected)
    this.optionalClient = axios.create({
      baseURL: API_BASE_URL,
      headers: {
        'Content-Type': 'application/json',
      },
      // Don't throw error for 404, treat it as valid response
      validateStatus: (status) => status < 500, // Accept all status codes < 500 (including 404)
    });
    ...
    // Add request interceptor for optional client (include auth token)
    this.optionalClient.interceptors.request.use(
      (config) => {
        const token = authService.getToken();
        if (token) {
          config.headers.Authorization = `Bearer ${token}`;
    ...
    // Add response interceptor for optional client
    // With validateStatus, 404 won't throw error, but we still handle other errors
    this.optionalClient.interceptors.response.use(
      (response) => response,
      (error) => {
        ...
        // For other errors (network errors, etc.), still reject
        return Promise.reject(error);
      }
    );
  ...
  // Get method that silently handles 404 (for optional resources)
  // Uses fetch API which doesn't automatically log errors to console like axios does
  async getOptional(endpoint, params = {}) {
    try {
      ...
      // Use fetch API - temporarily suppress console.error to prevent 404 logging
      // Note: Browser DevTools may still show 404 in Network tab (this is normal)
      const originalError = console.error;
      const originalWarn = console.warn;
      
      // Suppress console methods temporarily
      console.error = () => {};
      console.warn = () => {};
      ...
      try {
        ...
        // Restore console methods after fetch completes
        // Use setTimeout to ensure it happens after any potential browser logging
        setTimeout(() => {
      ...
      // Only network errors (not HTTP errors like 404) will reach here
      // For 404, fetch returns a response with status 404, not an error
      throw error;
    }
  }
